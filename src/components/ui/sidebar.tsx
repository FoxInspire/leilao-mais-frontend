   ...
   (
      ...
      const sidebarContent = (
         <div
            ...
            className="flex h-full w-full flex-col gap-6 bg-sidebar group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow dark:bg-dark-background-paper"
         >
            {children}
         </div>
      )
      ...
      const HeaderMenu = () => {
         ...
         return (
            <div className="fixed left-0 right-0 top-0 z-20 flex h-14 items-center justify-between rounded-b-lg bg-dark-background-paper px-6 dark:bg-primary-default">
               <button
                  onClick={() => {
                     if (isMobile) {
                        setOpenMobile(true)
...
>(({ className, ...props }, ref) => {
   ...
   return (
      <button
         ...
         className={cn(
            'absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-[1px] after:bg-sidebar-border hover:after:w-[2px] group-data-[side=left]:-right-4 group-data-[side=right]:left-0 dark:after:bg-[#1f252b] sm:flex',
...
>(({ className, ...props }, ref) => {
   ...
   return (
      <main
         ...
         ref={ref}
         className={cn(
            'relative grid w-full overflow-x-auto bg-white dark:bg-dark-background-paper',
            hasHeaderMenu ? 'mt-14' : '',
...
>(({ asChild = false, size = 'md', isActive, className, ...props }, ref) => {
   ...
   return (
      <Comp
         ...
         data-active={isActive}
         className={cn(
            'flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 text-sidebar-foreground outline-none ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 dark:hover:bg-[#fafafa0a] [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-sidebar-accent-foreground',
